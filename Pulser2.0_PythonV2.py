...
from files.functions import * # self written functions

#############################################################################
##########################-OPTIONS-##########################################
#Debug
debug = False
debug2 = False # More debug information
debug3 = False #Port selector for debugging
debug4 = False # Prints mapping
debug5 = False # Prints quadtype

#Minimum short circuit resistance, only needed if use_resistance=True
SC_upper_res = 100000
use_resistance = False

#Thresholds for the DC test
threshDCs = 220
threshDCi = 190

#Dictionairy containing thresholds for the AC test
trDic = {"QL1 S":(180,30), "QL2 S":(200,50), "QL3 S":(280,70), "QL1 P":(330,30), "QL2 P":(600,50), "QL3 P":(900,50), "QS1 S":(90,20), "QS2 S":(120,30), "QS3 S":(160,40), "QS1 P":(200,20), "QS2 P":(400,20), "QS3 P":(800,30)}
...
while(root.winfo_exists()):
    ...
    if debug5:
        if(selected_board[0]=='S'):
            print("Strip")
        elif(selected_board[0]=='P'):
            print("Pad")
        else:
            print("Error")
        print(selected_map[0:3])
    threshACs, threshACi = trDic["{} {}".format(selected_map[0:3], selected_board[0])]
    
    # prints the pandas array used for mapping
    if debug4: print(df)
    ...
    # Code responisble for position-printing in Errors-Sheet
    ...
    for channel in channels:
        if debug4: print(channel.gfz,selected_map)
    ...
    if True:
        ...
        arduino.timeout = 5
        # original value was 2, changed by Jacob
        time.sleep(2)
        ##Start comunication
    ...
    if print_CSV:
        ...
        # Excel setup
        outputDir = datetime.datetime.now().strftime("%Y_%b_%d_sTGC_"+selected_map_filename+"_"+selected_board)
        ...
        # creates format objects for color coding in the excel sheet
        ...
        ground= workbook.add_format()
        ground.set_bg_color('#cad5db')
        ground.set_align('center')
        ground.set_border(1)
        ...
        conectorDC.write(4, 0, 'Ground', ground)
        # Connector AC
        conectorAC = workbook.add_worksheet('ConnectorAC')
        conectorAC.set_column(0, 1, 20)
        ...
        conectorAC.write(4, 0, 'Ground', ground)

        # format the cells
        for channel in channels:
            # formatting for AC Test
            ...
            # highlight grounded contacts
            if channel.name == 'GND':
                channel.AC_color_code = ground
                channel.DC_color_code = ground

        # Fill the excel sheets
        counter1 = 1
        counter2 = 1
